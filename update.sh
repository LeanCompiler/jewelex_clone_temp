...
log "Syncing new files → ${APP_DIR} (logs & uploads excluded)"

# rsync the package into place, excluding persistent data dirs
rsync -a --delete \
  --exclude='logs' \
...
# 2) PM2 reload (zero-downtime). Fallback to full restart if config changed.
